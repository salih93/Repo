...
import 'package:sdrmobil/Screens/Raporlar/GrupBazindaSatislar/grup_bazinda_satislar_card.dart';
...
class GrupBazindaSatislar extends StatelessWidget {
  const GrupBazindaSatislar({Key key }) : super(key: key);
    ...
    return Obx(()=>WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text('Grup Bazında Satışlar', style: TextStyle(color: Colors.white, fontSize: size.height * kRaporOranBaslik),),
          centerTitle: true,
          ...
          slivers: [
            SliverToBoxAdapter(
              child: SizedBox(
                ...
                child:MusteriMalAnaliziBaslik(grupBazindaSatis: 1,),
              ),
            ),
            ...
            SliverFillRemaining(
              child: SizedBox(          
                ...
                child: FutureBuilder<List<GrupBazindaSatislarM>>(              
                  future:_controller.sayac.value>0 ? _controller.getGrupBazindaSatislar():_controller.getGrupBazindaSatislar(),              
                  builder: (BuildContext context, AsyncSnapshot<List<GrupBazindaSatislarM>> snapshot) {
                    if (snapshot.hasData) {
                      final datas = snapshot.data;
                      List<GrupBazindaSatislarCard> cards = [];
                      int index=0;
                      cards.clear();
                      ...
                      for (GrupBazindaSatislarM item in datas) {                      
                        itemIndex=0;
                        btoplamYaz=false;
                        bSonayaz=false;                            
                        oncekiIndex>index ? bSonayaz=true:bSonayaz=false;

                        if (cariKodu.toString().toLowerCase().trim()!=item.grupkodu.toString().toLowerCase().trim() && cariKodu!="")
                        { 
                        ...
                        if (toplamlar.where((element) => element.carikodu==item.grupkodu).length==0)
                        {
                          MusteriToplamlari toplam=MusteriToplamlari(
                            carikodu: item.grupkodu,
                            unvan: item.grupadi,
                            index: index,
                        ...
                        else
                        {
                          ...
                          {
                            toplamlar.where((element) => element.carikodu==item.grupkodu).forEach((elementItem) { 
                        ...
                        if (datas.length-1==index && !btoplamYaz)
                        {
                          MusteriToplamlari toplam=toplamlar.where((element) => element.carikodu==item.grupkodu).first;
                          ...
                          _controller.toplamNMiktar.value=toplam.toplamNMiktar;
                          _controller.toplamKdvsiz.value=toplam.toplamKdvsiz;
                          _controller.cariKodu.value=item.grupkodu.toString();
                          _controller.unvan.value=item.grupadi;
                          btoplamYaz=true;
                          bSonayaz=true;
                        }
                        ...
                        {
                          _controller.cariKodub.value=item.grupkodu; 
                          _controller.unvanb.value=item.grupadi; 
                        }
                        else
                        {
                        ...
                        cards.add(
                        GrupBazindaSatislarCard(item: item, index: index, btoplamYaz: btoplamYaz, 
                            bSonayaz:bSonayaz, 
